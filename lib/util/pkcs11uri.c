...
static SECStatus
pk11uri_InsertAttributeList(PK11URIAttributeList *attrs,
...
static SECStatus
pk11uri_InsertAttributeListEscaped(PK11URIAttributeList *attrs,
                                   ...
                                   PRBool allow_duplicate)
{
    ...
    ret = pk11uri_InsertAttributeList(attrs, name_copy, value_copy, compare_name,
                                      allow_duplicate);
    if (ret != SECSuccess) {
        goto fail;
    }
    ...
    for (i = 0; i < num_attrs; i++) {
        ...
        for (j = 0; j < num_attr_names; j++) {
            if (strcmp(name, attr_names[j]) == 0) {
                ret = pk11uri_InsertAttributeList(dest_attrs,
                                                  name, value,
                                                  compare_name,
                                                  allow_duplicate);
                if (ret != SECSuccess) {
    ...
    for (; *p != '\0'; p++) {
        ...
        for (i = 0; i < num_attr_names; i++) {
            if (name_length == strlen(attr_names[i]) &&
                ...
                ret = pk11uri_InsertAttributeListEscaped(attrs,
